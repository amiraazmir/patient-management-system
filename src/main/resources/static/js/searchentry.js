/**
 * table search, entries and pagination
 */
(function($) {
	"use strict";
	...
	$.fn.bdt = function(options, callback) {
		...
		return this.each(function() {
			...
			if (settings.showSearchForm == 1) {
				searchForm = $('<form/>').addClass(searchFormClass).attr(
						'role', 'form').append(
			...
			if (settings.showEntriesPerPageField == 1) {
				entriesPerPageField = $('<form/>').addClass('form-horizontal')
						.attr('id', 'page-rows-form').append(
								$('<label/>').addClass(
										'pull-left control-label').text(
										pageFieldText)).append(
			...
			obj.before($('<div/>').addClass('table-header').append(searchForm)
					.append(
			 ...
			 */
			obj.after($('<div/>').attr('id', 'table-footer').append(
					$('<div/>').addClass('pull-left table-info')
		...
		function addPages() {
			...
			$.each(new Array(pageCount), function(index) {
				...
				pages.append($('<li/>').addClass(additonalClass).data('page',
						(index + 1)).append($('<a/>').text(index + 1)));
			});
					...
					function() {
						$.each(tableBody.find("tr"), function() {
							...
							if (text.toLowerCase().indexOf(
								...
								$(this).hide().removeClass('search-item');
							} else {
								$(this).show().addClass('search-item');
							}
		...
		function setPageCount(tableBody) {
			if (activeSearch) {
				pageCount = Math.ceil(tableBody.children('.search-item').length
						/ pageRowCount);
			} else {
				pageCount = Math.ceil(tableBody.children('tr').length
						/ pageRowCount);
			}
		...
		function paginate(tableBody, page) {
			...
			var rows = (activeSearch ? tableBody.find(".search-item")
					: tableBody.find("tr"));
